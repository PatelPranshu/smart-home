...
async function initSettings() {
    ...
    window.toggleUpdatePreference = async () => {
        ...
        try {
            ...
            // Dismiss OTA toast if switching to auto (updates will push automatically)
            if (isAuto) {
                const otaToast = document.getElementById('ota-toast');
                if (otaToast) otaToast.remove();
            }

            showToast(isAuto ? 'Automatic updates enabled — pending updates will start now' : 'Manual update mode — you\'ll be notified', isAuto ? 'success' : 'info');
        } catch (err) {
