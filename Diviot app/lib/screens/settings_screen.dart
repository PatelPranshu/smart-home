...
class SettingsScreen extends StatefulWidget {
  ...
  final VoidCallback? onAddDeviceOpened;

  SettingsScreen({this.openAddDevice = false, this.onAddDeviceOpened});
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void didUpdateWidget(covariant SettingsScreen oldWidget) {
    ...
    if (widget.openAddDevice && !oldWidget.openAddDevice) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _showAddDeviceModal();
        widget.onAddDeviceOpened?.call();
      });
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _titleController.text = profileData['homeTitle'] ?? 'My Home';
        });
      }
    } catch (e) {}
  }
      ...
      ElevatedButton(
        onPressed: () async {
          try {
            ...
            // Socket will push updated device list automatically
          } catch(e) {
            showToast(context, e.toString().replaceAll('Exception: ', ''), isError: true);
          }
        },
  ...
  Widget _buildListTile({
    ...
    return ListTile(
      ...
      subtitle: subtitle != null ? Text(subtitle, style: TextStyle(color: isDark ? Colors.white60 : Colors.black54, fontSize: 14)) : null,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        title: Text('Settings', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 28, letterSpacing: -0.5)),
        toolbarHeight: 80,
        actions: [
          Padding(
            padding: const EdgeInsets.only(right: 16.0),
            child: GestureDetector(
              onTap: () => _showLogoutModal(authProvider),
